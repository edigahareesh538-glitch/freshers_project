...
async function fetchParticipants() {
    const response =await fetch("http://10.76.15.77:5000/participants");
...
async function searchByRoll() {
    ...
    try{
        const res = await fetch(`http://10.76.15.77:5000/participants/roll/${roll}`);
...
async function deleteUser(id) {
    ...
    const res= await fetch(`http://10.76.15.77:5000/participants/${id}`, {
        method: "DELETE"
    });
    const data=await res.json();
    alert(data.message);
...
async function saveUpdate() {
    ...
    const res = await fetch(`http://10.76.15.77:5000/participants/${currentUpdateId}`, {
        method: "PUT",
...
async function fetchTotalCount() {
    const res = await fetch("http://10.76.15.77:5000/participants/count");
    const data = await res.json();
...
document.getElementById("del_all_data").addEventListener("click", async () => {
    ...
    const res = await fetch("http://10.76.15.77:5000/participants", {
        method: "DELETE"
    });
